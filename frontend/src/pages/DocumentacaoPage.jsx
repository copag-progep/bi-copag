...
const FEATURES = [
  ...
  { icon: "📋", title: "Atribuições", desc: "Carteira com dias no setor e na atribuição atual, faixas selecionáveis e exportações" },
  ...
  { icon: "🤖", title: "Upload automático", desc: "Script Playwright às 18:17, com recuperação às 18:47 somente quando o upload principal não concluir" },
  { icon: "📨", title: "Relatório diário", desc: "E-mail automático seg–sex às 19:07 BRT com ativos, fluxo do dia por setor e alertas de processos críticos" },
...
const WORKFLOWS = [
  ...
  { name: "daily-upload.yml", freq: "Seg–Sex 18:17 + recuperação 18:47", desc: "Playwright headless: login no SEI, troca de setor, coleta todas as páginas, gera CSV e faz upload via API key. A recuperação só executa quando ainda não houve sucesso no dia." },
  { name: "daily-report.yml", freq: "Seg–Sex 19:07 BRT", desc: "Antes de enviar, executa check_daily_upload_success.py para confirmar sucesso do daily-upload no dia. Se estiver OK, coleta /api/reports/daily-summary e envia e-mail HTML compacto." },
        ...
        <div className="doc-content">
          ...
          <DocSection id="s04" num="04" eyebrow="Banco de dados" title="Modelo de dados">
            ...
            {[
              ...
              { name: "pauta_itens", desc: "Processos selecionados para acompanhamento em uma sessão de pauta.", rows: [["id","Integer PK",""],["sessao_id","FK → pauta_sessoes","Sessão da pauta"],["protocolo / setor / entrada_setor","—","Identifica o processo e a permanência acompanhada"],["prazo","Date","Prazo dado pela gestão para conclusão do processo"],["dias_no_setor / score_risco / nivel_risco","—","Snapshot do risco no momento da inclusão"],["assigned_to / assigned_by","FK → users","Responsável e administrador que atribuiu"],["status","String(30)","pendente, em_acompanhamento, saiu_do_setor, resolvido_manual ou arquivado"],["nota_admin","Text","Orientação da gestão"],["data_status / resolucao_automatica","—","Data e origem da resolução"]] },
          ...
          <DocSection id="s05" num="05" eyebrow="API" title="Backend e endpoints">
            ...
            {[
              ...
              { group: "Analytics", endpoints: [
                ...
                ["GET", "/api/analytics/attributions", "Carteira com dias no setor e na atribuição atual (paginado, filtros, ordenação)"],
              ...
              { group: "Pauta Prioritária", endpoints: [
                ...
                ["PATCH", "/api/pauta/itens/{id}", "Atualiza prazo, nota da gestão, responsável ou status do item (admin)"],
          ...
          <DocSection id="s10" num="10" eyebrow="Operação" title="Manutenção do dia a dia">
            <Callout icon="🤖">
              O upload diário é <strong>automático às 18:17 BRT</strong>, com recuperação às
              <strong> 18:47</strong> caso a primeira execução não tenha sucesso. Se falhar,
              um e-mail de alerta é enviado automaticamente. Intervenção manual só é necessária
              em casos excepcionais. O relatório diário das 19:07 também verifica esse sucesso antes
          ...
          <DocSection id="s12" num="12" eyebrow="Evolução" title="Histórico de funcionalidades">
            {[
              ...
              { title: "Automação (Bloco 4)", items: ["API key para uploads sem JWT","Script SEI Scraper (Playwright headless): login, troca de setor por JS, coleta todas as páginas","Workflow daily-upload principal (18:17 BRT) e recuperação idempotente (18:47 BRT)","Workflow daily-report (19:07 BRT) bloqueado por check_daily_upload_success.py quando o upload do dia não concluiu com sucesso","Workflow weekly-report (sexta 20:00 BRT)","Script de alertas com anti-spam (não envia se sem críticos)","Workflow critical-alerts (sexta 21:00 BRT)"] },
