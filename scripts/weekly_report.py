#!/usr/bin/env python3
"""
Relatório gerencial semanal — AnalyticSEI
=======================================
Coleta dados da API do AnalyticSEI via API key, gera um e-mail HTML com os
...
# ---------------------------------------------------------------------------
# Coleta de dados da API do AnalyticSEI
# ---------------------------------------------------------------------------
...
def fetch(path: str, **params) -> dict:
    """Chama um endpoint analítico do AnalyticSEI usando API key, com retry automático."""
        ...
        <tr>
          <td>
            <span style="font-family:{_FONT};font-size:.65rem;font-weight:800;
                         ...
                         color:rgba(254,187,18,.85)">AnalyticSEI &nbsp;·&nbsp; COPAG &nbsp;·&nbsp;
                         PROGEP &nbsp;·&nbsp; UFC</span>
          </td>
        ...
        <tr>
          <td style="font-family:{_FONT};font-size:.72rem;color:#9a9fc0;line-height:1.7">
            ...
            <a href="https://bi-copag.vercel.app"
               style="color:#273168;font-weight:700;text-decoration:none">AnalyticSEI</a>
...
def send_email(html: str) -> None:
    ...
    msg["Subject"] = f"📊 AnalyticSEI — Relatório semanal {sem_label}"
    msg["From"]    = gmail_user
    msg["To"]      = recipients
...
if __name__ == "__main__":
    print("Coletando dados do AnalyticSEI...")
    _warmup()
